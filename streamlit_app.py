...
try:
    ...
    with c3:
        st.write(" ")
        # REPLACED JUMP WITH SORT: This is 100% reliable compared to scrolling
        view_order = st.radio("Display Order:", ["Oldest First (1-9)", "Newest First (9-1)"], horizontal=True)
    ...
    # --- SORTING LOGIC ---
    # Instead of jumping, we flip the data so the "Last" record is at the top
    if view_order == "Newest First (9-1)":
        df = df.sort_values(by="ID Number", ascending=False)
    else:
        df = df.sort_values(by="ID Number", ascending=True)

    # Styling with forced Sticky Header
    st.markdown("""
        <style>
            ...
            /* Ensure the Streamlit container doesn't hide the sticky header */
